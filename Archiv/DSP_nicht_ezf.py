...
# ===== Parameter anpassen =====
input_file     = "C:/eigene Programme/VS_Code_Programme/HKA/DSP/Inputsignale/Interleaved_drum_500_8k_rand.mp3"  # dein Testsignal
frame_duration = 0.001   # Sekunden pro Frame (Experimentell: 5 ms funktioniert)
percentile     = 80     # wie “streng” der Schwellwert ist (70–90 testen)
min_gap_s      = 0.01   # Minimalabstand zwischen Wechseln ( 9 ms da min länge 10 ms sind)
...
def assign_segments_to_sources(centroids_hz, frame_duration, change_frame_indices, cluster_threshold_hz=500.0):
    """
    ...
    min_segments_per_cluster = 3  # <= hier kannst du spielen (2 oder 3 ist meist gut)
...
#--------------------------------------------------------------------------------------
    ...
    n_frames = len(centroids_hz)
    '''
    # Prio-1 bis -3-Zeitpunkte zusammenfassen und in Frame-Indizes umrechnen
    prio_times_all = np.concatenate([prio1_times, prio2_times, prio3_times]) if (
        len(prio1_times) + len(prio2_times) + len(prio3_times)
    ) > 0 else np.array([])
    '''
    
    # Prio-2 und -3-Zeitpunkte zusammenfassen und in Frame-Indizes umrechnen
    prio_times_all = np.concatenate([prio2_times, prio3_times]) if (len(prio2_times) + len(prio3_times)) > 0 else np.array([])

    frame_boundaries = []
    for t in prio_times_all:
        f_idx = int(round(t / frame_duration))
