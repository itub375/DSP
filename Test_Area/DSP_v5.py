...
class Config:
    ...
    IGNORE_ONLY_ENERGY_AND_JUMP: bool = True
    ...
    MIN_SEGMENT_DURATION_S: float = 0.01     # harte Untergrenze
    ...
    NUM_SIGNALS: Optional[int] = 3 # None        # z.B. 2 oder 3; None => auto (heuristisch)
...
def plot_results(cfg: Config,
    ...
    plt.figure()
    plt.plot(t_plot, y_plot, linewidth=0.8, color="black", label="Signal")

    # Hilfsfunktion für farbige Vlines + Legend-Handling
    def vlines_from_peaks(p, color, ls, label_once: str):
        first = True
        for idx in p:
            ts = float(idx * hop_len / sr)
            if tmax is None or ts <= tmax:
                plt.axvline(ts, color=color, linestyle=ls, linewidth=1.0,
                            label=(label_once if first else None))
                first = False

    # Methoden-Peaks farbig
    vlines_from_peaks(peaks["centroid"], color="tab:orange", ls="--", label_once="Centroid peaks")
    vlines_from_peaks(peaks["energy"],   color="tab:green",  ls="-.", label_once="Energy peaks")
    vlines_from_peaks(peaks["jump"],     color="tab:purple", ls=":",  label_once="Jump peaks")
    vlines_from_peaks(peaks["shape"],    color="tab:cyan",   ls=(0, (3, 1, 1, 1)), label_once="Shape peaks")

    # Final genutzte Segmentgrenzen (Boundaries) deutlich hervorheben
    first_b = True
    for b in boundaries:
        if b == 0.0:
            continue
        if tmax is not None and b > tmax:
            continue
        plt.axvline(b, color="tab:red", linewidth=1.6, label=("Final boundaries" if first_b else None))
        first_b = False

    # Ground Truth Marker (optional)
    if cfg.TRUE_SEGMENT_DURATION_S is not None:
        gt = cfg.TRUE_SEGMENT_DURATION_S
        x = gt
        first_gt = True
        while x < (tmax if tmax else t[-1]):
            plt.axvline(x, color="gray", linestyle=(0, (1, 3)), linewidth=1.0,
                        label=("Ground truth" if first_gt else None))
            first_gt = False
            x += gt

    plt.title("Waveform + Detected Change Points")
    plt.xlabel("Time (s)")
    ...
    for b in boundaries:
        if tmax is None or b <= tmax:
            plt.axvline(b, color="tab:red", linewidth=1.2, label=("Final boundaries" if first_b2 else None))
            first_b2 = False
