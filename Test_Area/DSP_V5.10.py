"""
Audio Segmentation v5.8 - 100% NUR FLUX
Änderungen:
- Alle Features außer Flux entfernt
- RMS komplett entfernt (kein Amplitude-Check mehr, falls nicht explizit aktiviert)
- Segmentation erfolgt ausschließlich anhand von Spectral Flux PEAKS
- Nur 3 Plots: Waveform, Flux (RAW), Detection Score
"""
import os
import numpy as np
...
class Config:
    ...
    # ⭐ DEAKTIVIERT: Keine Amplitude-Filterung mehr
    MIN_AMPLITUDE_THRESHOLD: float = 0.0  # 0 = deaktiviert
    
    CHANGE_DETECTION_CFAR: bool = True
...
def extract_segment_features(y: np.ndarray, sr: int, 
    ...
    """
    Extrahiert Features für jedes Segment - NUR FLUX
    """
    ...
    for start_t, end_t in segments:
        ...
        features.append([flux])  # Nur Flux, kein RMS mehr
    
    return np.array(features)

def cluster_segments(features: np.ndarray, n_clusters: int = None) -> np.ndarray:
    """
    Clustert Segmente NUR basierend auf Flux (1D-Clustering)
    """
    
    if len(features) == 0:
        return np.array([])
    
    # Normalisierung (auch wenn nur 1D, bleibt Struktur konsistent)
...
def plot_results(y: np.ndarray, sr: int, cfar: bool, features: Dict, save_path, 
    ...
    """Visualisiert Ergebnisse - NUR FLUX (3 Plots)"""
    ...
    # Markiere ungültige Bereiche (falls MIN_AMPLITUDE_THRESHOLD > 0)
    ...
    ax.set_title('Waveform + Segmentation (100% FLUX-basiert)')
    ...
    # 2. Spectral Flux (EINZIGES FEATURE)
    ax = axes[1]
    ...
    ax.set_ylabel('Flux')
    ax.set_title('⭐ Spectral Flux (RAW)')
    ax.grid(True, alpha=0.3)
    
    # 3. Flux-basierter Detection Score (= normalisierter Flux)
    ax = axes[2]
    if cfar:
        if max_seconds:
...
def main():
    ...
    for idx, audio_file in enumerate(audio_files):
        ...
        cfg.OUT_DIR = "out/out_v5.10_CFAR/" + str(base_name) + "/"  

        print("="*60)
        print("Interleaved Audio Segmentation v5.10 - 100% FLUX")
        print("Kein RMS - Reine Flux-basierte Erkennung")
        print("="*60)
        ...
        # Features
        print(f"\n[2/6] Computing features (NUR FLUX - kein RMS)...")
        print(f"  → Window: {cfg.WINDOW_MS} ms")
        print(f"  → Hop: {cfg.HOP_MS} ms")
        if cfg.MIN_AMPLITUDE_THRESHOLD > 0:
        ...
        if cfg.MIN_AMPLITUDE_THRESHOLD > 0:
            ...
            print(f"  → Frames: {n_total} (alle gültig)")
        ...
        for i in range(n_signals):
            ...
            print(f"  → Signal {chr(ord('A')+i)}: {count} segments, Flux={mean_flux:.3f}")
