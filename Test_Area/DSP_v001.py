...
def reconstruct_signals_compact_fade(
    y: np.ndarray,
    sr: int,
    segments: List[Tuple[float, float]],
    labels: np.ndarray,
    ...
    """
    Rekonstruiert Signale pro Label ohne Stille.
    Segmente werden zusammengezogen und mit Crossfades überblendet.
    """
 
    os.makedirs(out_dir, exist_ok=True)
 
    if len(segments) != len(labels):
        raise ValueError("segments und labels müssen gleich lang sein")
    ...
    for label in unique_labels:
        ...
        for (start_t, end_t), seg_label in zip(segments, labels):
            ...
            end_idx = min(len(y), int(end_t * sr))
        ...
        # erstes Segment unverändert
        reconstructed = chunks[0].copy()
 
        for next_chunk in chunks[1:]:
            # Fade-Länge an Segmentgröße anpassen
            f = min(fade_len, len(reconstructed), len(next_chunk))
 
            if f > 0:
                fade_out = np.linspace(1.0, 0.0, f, endpoint=False)
                fade_in = np.linspace(0.0, 1.0, f, endpoint=False)
 
                # Überblendbereich
                overlap = (
                    reconstructed[-f:] * fade_out +
                    next_chunk[:f] * fade_in
                )
                ...
                reconstructed = np.concatenate([reconstructed, next_chunk])
...
def main():
    ...
    for a in audio_files:
        ...
        exported = reconstruct_signals_compact_fade(y, sr, segments, labels, cfg.OUT_DIR, cfg.EXPORT_FORMAT, cfg.FADE_IN)
