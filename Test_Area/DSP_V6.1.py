...
Mit Bandpass-Vorfilterung (20 Hz - 20 kHz)

ÄNDERUNG ZU V5.2:
Es hat jetzt einen Band pass filter
(HINWEIS: Funktioniert anscheinend nicht)

NEU:
        Config      : Attribute für den Bandpassfilter
        Main        : Neuer Schritt 1.5 zum anwenden des Filters
        apply_bandpass_filter : Neue Methode für den Filter
...
class Config:
    ...
    # Bandpass Filter
    BANDPASS_LOW: float = 20.0   # Untere Grenzfrequenz (Hz)
    BANDPASS_HIGH: float = 30000.0  # Obere Grenzfrequenz (Hz)
    FILTER_ORDER: int = 4  # Butterworth-Filter-Ordnung
    ...
    # Change Detection
    CHANGE_THRESHOLD_PERCENTILE: float = 92.0  # Schwelle für Change-Score (höher = weniger detections)
    MIN_SEGMENT_MS: float = 30.0    # Minimale Segmentlänge (ms)
    MERGE_TOLERANCE_MS: float = 10.0 # Toleranz für Segment-Merge
...
def apply_bandpass_filter(y: np.ndarray, sr: int, lowcut: float, highcut: float, order: int = 4) -> np.ndarray:
    """
    Wendet Butterworth-Bandpass-Filter an
    """
    nyq = 0.5 * sr
    low = lowcut / nyq
    high = highcut / nyq
    ...
    if low >= high:
        print(f"WARNUNG: Bandpass-Grenzen ungültig ({lowcut}-{highcut} Hz), überspringe Filter")
        return y
    ...
    try:
        ...
        print(f"WARNUNG: Bandpass-Filter fehlgeschlagen: {e}")
        return y
...
def find_boundaries(change_score: np.ndarray, threshold: float, 
    ...
    Verbesserte Version mit stärkerer Filterung
    """
    
    # Finde Peaks über Threshold mit besserer lokaler Maximum-Bedingung
    candidates = []
    window = 3  # Betrachte ±3 Frames für lokales Maximum
    
    for i in range(window, len(change_score) - window):
        if change_score[i] > threshold:
            # Prüfe ob es ein lokales Maximum in größerem Fenster ist
            local_region = change_score[i-window:i+window+1]
            if change_score[i] == local_region.max():
                candidates.append((times[i], change_score[i]))
    
    # Sortiere nach Score (stärkste zuerst)
    candidates.sort(key=lambda x: x[1], reverse=True)
    
    # Greedy: Nimm nur Peaks die mind. min_segment_s auseinander liegen
    boundaries = [0.0]
    for t, score in candidates:
        # Check Abstand zu allen bereits gewählten boundaries
        if all(abs(t - b) >= min_segment_s for b in boundaries):
            boundaries.append(t)
...
def main():
    ...
    print(f"\n[1.5/6] Applying Bandpass Filter: {cfg.BANDPASS_LOW}-{cfg.BANDPASS_HIGH} Hz")
    y = apply_bandpass_filter(y, sr, cfg.BANDPASS_LOW, cfg.BANDPASS_HIGH, cfg.FILTER_ORDER)
